...
def predict_image(image: Image.Image):
    ...
    # Ghi lại inference time
    inference_time.set(end_time - start_time)

    # Tính confidence score (softmax)
    probs = torch.softmax(output, dim=1)
    max_prob = probs.max().item()
    confidence_score.set(max_prob)

    pred = torch.argmax(output, 1).item()
    return class_names[pred]

# 7. Metrics (cần prometheus_client)
from prometheus_client import Gauge
inference_time = Gauge('model_inference_time_seconds', 'Time for model inference')
